...
func NewAgentLoop(cfg *config.Config, bus *bus.MessageBus, provider providers.LLMProvider) *AgentLoop {
	...
	searchCfg := cfg.Tools.Web.Search
	switch searchCfg.Provider {
	case "google":
		toolsRegistry.Register(tools.NewGoogleSearchTool(searchCfg.Google.APIKey, searchCfg.Google.CX, searchCfg.MaxResults))
	default: // "brave" or empty
		toolsRegistry.Register(tools.NewBraveSearchTool(searchCfg.APIKey, searchCfg.MaxResults))
	}
